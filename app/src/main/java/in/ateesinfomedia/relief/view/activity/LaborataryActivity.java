...
import in.ateesinfomedia.relief.view.adapter.LaborataryAdapter;
import in.ateesinfomedia.relief.widgets.searchdialog.OnSearchItemSelected;
import in.ateesinfomedia.relief.widgets.searchdialog.SearchListItem;
import in.ateesinfomedia.relief.widgets.searchdialog.SearchableDialog;
...
//    private List<DoctorsModel> doctorsList = new ArrayList<>();
    private List<LaboratoryModel> laborataryList = new ArrayList<>();
//    private List<DoctorsModel> filteredDoctorsList = new ArrayList<>();
    ...
    private List<SearchListItem> searchLaboratoryList = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState); //must be called before setContentView(...)
...
//            window.setNavigationBarColor(this.getResources().getColor(android.R.color.transparent));
        ...
        mRecyclerViewDoctors = (RecyclerView) findViewById(R.id.recycler_laboratory);

        laborataryList = LaborataryList;
        constituencyList = ConstituencyList;
        LoadLaborataories();

        mRecyclerViewDoctors.setNestedScrollingEnabled(true);
        mAdapter = new LaborataryAdapter(this,laborataryList,this);
...
//        ShimmerRecyclerView.LayoutManager mLayoutManager = new LinearLayoutManager(this);
        ...
        mEtConstituency.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                SearchableDialog searchableDialog = new SearchableDialog(LaborataryActivity.this, searchConstituencyList, "Select Constituency");
...
//                loadDepartmentList();
                LoadLaborataories();
                SearchableDialog searchableDialog = new SearchableDialog(LaborataryActivity.this, searchLaboratoryList, "Select Laboratory");
                searchableDialog.setOnItemSelected(new OnSearchItemSelected() {
                    @Override
                    public void onClick(int i, SearchListItem searchListItem) {
        ...
        mImSearch.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (mEtDepartment.getText().toString().isEmpty() && mEtConstituency.getText().toString().isEmpty()){
                    Toast.makeText(LaborataryActivity.this, "Please add any credential", Toast.LENGTH_SHORT).show();
...
//                        doFilterListOne("department");
    ...
    private void LoadLaborataories() {
        searchLaboratoryList.clear();
        for (int i = 0;i<laborataryList.size();i++){
            String id = laborataryList.get(i).getId();
            String name = laborataryList.get(i).getName();

            SearchListItem searchListItem = new SearchListItem(Integer.valueOf(id),name);
            searchLaboratoryList.add(searchListItem);
...
//        if (text.length() != 0){
        if (dep_name!=null && con_name != null){
            for(LaboratoryModel d: laborataryList){
...
//        }
    ...
    public void itemClicked(int position, View view, View view1, Object o) {
        ...
        Intent intent = new Intent(LaborataryActivity.this,LaborataryDetailsActivity.class);
